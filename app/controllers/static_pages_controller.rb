  ...
  def pullable_rooms_number (dorm)
    if current_user
      if current_user.student
        bumpable_assignments = RoomAssignment.select { |r| r.room.dorm == dorm && r.pull && current_user.student.outranks(r.pull.student) }
        room_id_array = []
        for ra in bumpable_assignments
          room_id_array.push(Room.find(ra.room_id))
        end        
        @p = room_id_array.uniq{|x| x.id}.select{|x| Dorm.find(x.dorm_id).name == dorm.name}.count
      end
    end
  end
